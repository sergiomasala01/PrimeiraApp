...
namespace PrimeiraApp.Models
{
    public class Aluno
    {
        ...
        //[RegularExpression(@"^[\w-\.]+@([\w-]+\.)+[\w]{2,4}$", ErrorMessage = "O campo {0} está em formato invalido")] expressoes regulares tambem podem ser utilizadas
        public string? Email { get; set; }

        [Required(ErrorMessage = "O campo {0} é obrigatório")]
        [Display(Name = "Confirme o e-mail")]
        [Compare("Email", ErrorMessage = "Os e-mails informados não são identicos")]
        [NotMapped] //nao vai mapear o campo para o database
